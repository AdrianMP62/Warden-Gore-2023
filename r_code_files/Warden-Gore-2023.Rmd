...
\* Warden_Gore_by_suite_06_23.csv (System, SurveyType, SiteName, SiteCode, Subsite Date, CommonName, SpeciesCode, Count) (e.g. Warden, aerial, Neridup Suite, WRP001, combined, 11/10/2006, Great Egret, greg, 1).
Note some double counts for Oct 2006 and Feb 2015 removed.
...
pacman::p_load("plot3D","scatterplot3d","plot3Drgl","gridExtra","vegan", "tidyr","ggplot2","openxlsx2", "rgl", "ggrepel", "dplyr", "plyr", "scales", "ggpubr", "stringr","reshape2", "knitr", "zoo", "car","reshape2","readxl","lubridate","Rcpp")
```
...
####box plots These box plots show the abundance of the top 8 species contributiong to waterbird communities being different in spring and summer (Gore and Warden systems combined).
...
```{r}
#spring
SurvTime.sp <- bioenv(Warden.spr.m ~ Surv.Pipe.spr[, "Days"], method = "pearson", index = "bray", ) #bioenv to testfor effect of pipeline operation days
SurvTime.sp
...
#summer
SurvTime.su <- bioenv(Warden.sum.m ~ Surv.Pipe.sum[, "Days"], method = "pearson", index = "bray", ) #bioenv to testfor effect of pipeline operation days
SurvTime.su
...
databreaks <- seq(as.Date("2004-01-01"), as.Date("2023-03-01"), by="3 month")
...
```{r}
ggplot(rainfall, aes(x = Month.2, y = CDMM, colour=Station)) + geom_line() + theme (axis.text.x=element_text(angle=90)) + scale_x_date(breaks=databreaks, labels = labels) + geom_bar(aes(x=year, y=course),stat="identity", fill="cyan",colour="#006000")
```
...
```{r}
sp.id <- c("Australian Shelduck","Hoary-headed Grebe","Yellow-billed Spoonbill","Australian Pelicans","Grey Teal")
sp.selected <- Species.sum[Species.sum$CommonName == sp.id, ]
...
Warden.spr.splots
Warden.spr.splots$Survey2 <- format(as.Date(Warden.spr.splots$Date), "%b-%Y")
...
p <- ggplot(Warden.spr, aes(x=Date, y=Count)) + geom_col()
p + facet_wrap( ~ CommonName)
...
```{r}
WGdata <- read.csv("./raw_data/Warden_Gore_by_suite_06_23.csv")
